...
"""
INSERT ONE OF TWO BELOW CELLS HERE
"""

ccdlog = np.genfromtxt('ccdlog.csv',delimiter=',')
ccdlog = ccdlog[ccdlog != 10]
ccd_data = []        
for i in range(len(ccdlog)):
    item = ccdlog[i]*20
    if (ccd_lower*20) < item < (ccd_upper*20):
        item2 = (-20)*ccdlog[i]
        ccd_data = np.append(ccd_data,item)
        ccd_data = np.append(ccd_data,item2)
n, bins, patches = plt.hist(ccd_data, bins = 100)
plt.title('All lens transformed photons')
plt.xlabel('Position along CCD plane/mm')
plt.ylabel('Counts')
plt.savefig('transformedhist.png',dpi = 1000)
plt.show()
...
dumb = 270/(40*ccd_upper)
dumber = 6860/dumb
mm = pixel/dumb -dumber
relative_intensity = intensity/maximum
p = np.polyfit(mm,relative_intensity, 4)
real_fit = p[0]*(mm**4) + p[1]*(mm**3) + p[2]*(mm**2) + p[3]*mm + p[4]

bins_mean = [0.5 * (bins[i] + bins[i+1]) for i in range(len(n))] # Central x values of the bins
bins_mean2 =[]
for i in range(len(bins_mean)):                                            # Removes sides so the data is within bounds of experimental
    value = bins_mean[i]
    bins_mean2 = np.append(bins_mean2, value)          # Converts to equal axes
mean_n = np.average(n)
max2 = max(n)
normalised_n=[]
...
plt.show()
np.savetxt("relative_intensity.csv", relative_intensity, delimiter=",")
