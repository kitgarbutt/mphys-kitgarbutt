...
def ray_trace(alpha_x,alpha_y):
    ...
    leaf_width = 0.75
    N = 100000 # Number ophotons included in the interaction
    spec_prob = 0.01
    abs_spec = 0.01
    lens_upper = np.sqrt(1-(camera_position**2))
    diffuser_limit_l = x_baffle+0.01
...
def biggerfish(alpha_x,alpha_y):
    quigon = 0
    jarjar = []
    while quigon < 10:
        jarjar = np.append(jarjar, ray_trace(alpha_x,alpha_y))
        quigon = quigon + 1
    return (np.average(jarjar), np.var(jarjar))
        


guess = [0.85,0.075]
xlist = np.arange(0.76,0.795,0.005)
ylist = np.arange(0,0.035,0.005)
newx = []
newy = []
for i in range(len(xlist)):
    ...
    for i in range(len(ylist)):
        itemy = ylist[i]
        newx=np.append(newx,itemx)
        newy=np.append(newy,itemy)

double = list(zip(newx,newy))
np.savetxt("double.csv", double, delimiter=",")
chilist=[]
stddevlist=[]
pbar = tqdm_notebook(total = len(double))
for i in range(len(double)):
    line = double[i]
    alpha_x = line[0]
    alpha_y = line[1]
    #biggerfishoutput = biggerfish(alpha_x,alpha_y)
    chisquared,stddev = biggerfish(alpha_x,alpha_y) #biggerfishoutput[0]
    chilist=np.append(chilist,chisquared)
    stddevlist=np.append(stddevlist,stddev)
    pbar.update(1)
pbar.close()
optimization_data = list(zip(newx,newy,chilist,stddevlist))
np.savetxt("optimization_data_plot2.csv", optimization_data, delimiter=",")
    
"""
result = optimize.minimize(ray_trace,guess, method='L-BFGS-B', bounds =[(0.751,0.9),(0.05,0.2)],options={'eps':0.01, 'ftol': 0.01})
if result.success:
