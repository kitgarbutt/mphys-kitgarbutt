...
while counter < N:
    ...
    elif surface_counter == 2:                            # If first interaction is with circle, call relevant intersection and interaction 
                                                          #functions
    ...
    while absorbed == 0:
        ...
        if pos[1] == camera_position:
            ...
            logx = np.append(logx,pos[0])
            logy = np.append(logy,pos[1])# Same as above but for camera interaction
...
plt.hist2d(position_record, direction_record, bins=(20, 20), cmap=plt.cm.jet)
...
plt.savefig('2dhist.png',dpi = 1000)
plt.colorbar()
plt.show()
...
plt.xlim(0,0.3)
plt.ylim(0,3.14)
plt.savefig('2dkde.png',dpi = 1000)
plt.show()